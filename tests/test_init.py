...
def test_init_kaiming_uniform():
    np.random.seed(42)
    np.testing.assert_allclose(kim.init.kaiming_uniform(3,5).numpy(),
...
def test_init_kaiming_normal():
    np.random.seed(42)
    np.testing.assert_allclose(kim.init.kaiming_normal(3,5).numpy(),
...
def test_init_xavier_uniform():
    np.random.seed(42)
    np.testing.assert_allclose(kim.init.xavier_uniform(3, 5, gain=1.5).numpy(),
...
def test_init_xavier_normal():
    np.random.seed(42)
    np.testing.assert_allclose(kim.init.xavier_normal(3, 5, gain=0.33).numpy(),
